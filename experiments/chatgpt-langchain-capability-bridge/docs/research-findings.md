...
- LangSmith recorded one native Thread with 29 root tool traces, zero errors,
  and 16 additional server-observed invocations beyond visible calls;
- official tunnel logs and OTel metrics showed 31 unique `tools/call` commands
  in the project window, and all 31 matched LangSmith project root traces
  one-to-one. Twenty-nine belonged to C0, one was an unthreaded manifest call,
  and one belonged to a pre-existing workstream. The 16 additional C0 calls
  therefore arrived from the OpenAI host/control-plane boundary before bridge
  execution rather than being generated by the bridge or Agent Server;
- the additional C0 calls were only workspace open/status, context read, and
  file read. The two edits and three executions were each invoked exactly once;
