...
`codex_session_metrics` incrementally indexes the selected protected rollout,
persisting counts and digests without transcript content. Response schema 2
separates `tools` (model request records, including outer `exec`) from
`nativeActivities` (completed native items/end events). Raw Codex 0.153.4
`item_completed` records expose command, MCP, file and dynamic-tool activities;
`Extension` kind `web.search` includes web search/open/find activity. The public
[App Server item lifecycle](https://learn.chatgpt.com/docs/app-server) uses
different item names from those persisted records. JavaScript text inside an
`exec` request is never treated as proof that a nested call ran. Failed or denied
completed items count as observations, not successful external effects. File
events do not enumerate every filesystem mutation.

Absent MCP/web/file counts are `null`, not verified zero. `coverage` reports
unsupported/malformed records, attribution and indexing bounds. Repeated native
ID/category/turn combinations are counted once, including across incremental
reads; unmatched legacy/current representations may still overlap. The old
aggregate cache is automatically reindexed at this schema generation. A partial
last JSONL line is retained for a later append, rather than silently lost.
`completeAtObservation` means the byte cursor reached the observed selected
file's EOF; it does not certify telemetry completeness or read referenced prior
history. Same-inode rewrites require `forceReindex`; the normal cursor assumes
append-only native rollout files.

Native `token_usage_record` cumulative thread/turn snapshots replace duplicate
legacy samples. Thread totals may include earlier paginated history; legacy
`token_count` snapshots are not claimed as lifetime totals. Completed-turn token
averages require native snapshots for all indexed completed turns; otherwise
they are unavailable. Tool averages use attributable completed-turn request
records. Duration statistics use all indexed completed turns with timing, not
the displayed page, excluding active/interrupted work. Each denominator and
source is disclosed. These observations are neither billing/goal-budget totals
nor a measure of whole-mission quality. Validated 0.149 and 0.153.4 profiles also
expose native thread-scoped usage; the 0.147 account route remains account-scoped.

Protocol effects remain exact-version fail-closed. The 0.153.4 profile is bound
to the non-experimental App Server schemas generated by the installed Codex
binary. Additive request and notification methods do not authorize themselves;
unknown future protocol versions remain read-only until their exact profile is
validated. Codex 0.153.4 also distinguishes terminal `writeStdin` approvals
from command approvals. The gateway projects that distinction but will only
decline or cancel terminal-input approvals because the approval request does
not expose the input bytes needed for a model-facing accept decision.
