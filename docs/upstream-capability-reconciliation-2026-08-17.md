...
| `zes-codex-workspace-inspection.ts` | Read-only projection of the exact allowlisted Codex AOQ worktree | The generic native Codex gateway now owns current and multi-session inspection | `KEEP_CUSTOM` only for bounded forensic/worktree access; the superseded fixed-thread session adapter was retired on 2026-08-21 |
